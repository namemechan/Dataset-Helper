...
### 6. 데이터셋 분석 (Dataset Analyzer) [New] 
...
- **정밀 버킷팅 로직:** 단순 해상도 반올림 방식이 아닌, 설정된 기준 해상도의 면적(Area)을 유지하며 원본 비율에 가장 적합한 버킷을 찾아 할당하는 정밀 분석 로직이 적용되었습니다.
- **실시간 계산 갱신:** 설정을 변경한 후 "분석 (계산 갱신)" 버튼을 누르면, 폴더를 다시 스캔할 필요 없이 메모리에 저장된 이미지 정보를 바탕으로 즉시 버킷 분포와 낭비율이 재계산됩니다.
- **최적 리핏(Repeat) 추천:** 설정된 배치 사이즈와 각 폴더의 데이터셋 수를 비교하여, 낭비되는 슬롯을 최소화할 수 있는 권장 리핏 값을 자동으로 계산합니다.
- **예상 스텝 및 낭비율 계산:** 
    - **이론적 스텝:** 버킷 손실이 없는 순수 계산상의 스텝 수입니다.
    - **실제 예상 스텝:** 버킷 분류 및 배치 묶음 시 발생하는 낭비를 포함한 실제 학습 스텝 수입니다.
    - **평균 낭비율:** 버킷 규격 불일치로 인해 발생하는 학습 효율 저하를 퍼센트로 표시합니다.
- **지능형 리핏 설정:** 
    - **최적 리핏 설정:** 데이터가 많은 폴더는 약수를, 적은 폴더는 배수를 활용해 낭비율 0%에 가깝게 리핏을 자동 조정합니다.
    - **개별 수정:** 분석된 표에서 특정 폴더의 리핏을 직접 더블클릭하여 수정할 수 있습니다.
- **결과 CSV 출력:** 분석된 전체 요약 정보와 폴더별 상세 내역을 엑셀에서 열 수 있는 CSV 파일(`utf-8-sig`)로 출력하여 관리할 수 있습니다.
- **강력한 정렬:** 폴더 이름, 데이터셋 수, 낭비율 등을 기준으로 오름차순/내림차순 정렬이 가능하며 데이터 갱신 시에도 정렬 상태가 유지됩니다.
